...
echo -e "$OKRED    metafiks   $RESET"
echo -e "$RESET"
echo -e "$OKORANGE + -- --=[ https://metafikssecurity.com $RESET"
echo -e "$OKORANGE + -- --=[ metafiks CE by @metafiks $RESET"
echo ""

INSTALL_DIR=/usr/share/metacy
LOOT_DIR=/usr/share/metacy/loot
PLUGINS_DIR=/usr/share/metacy/plugins
GO_DIR=~/go/bin

echo -e "$OKRED[>]$RESET This script will install metafiks under $INSTALL_DIR. Are you sure you want to continue? (Hit Ctrl+C to exit)$RESET"
if [[ "$1" != "force" ]]; then
	read answer
fi
...
apt install -y curl
apt install -y nmap
apt install -y php8.3.0
apt install -y php3.3.0-curl
apt install -y hydra
...
rm -f /usr/share/metacy/templates/active/CVE*
...
wget https://raw.githubusercontent.com/janmasarik/resolvers/master/resolvers.txt -O /usr/share/metacy/wordlists/resolvers.txt
...
chmod +x $INSTALL_DIR/metacy
chmod +x $PLUGINS_DIR/Goohak/goohak
rm -f /usr/bin/dirsearch
ln -s $INSTALL_DIR/metacy /usr/bin/metacy 2> /dev/null
...
ln -s /usr/share/metacy /metacy 2> /dev/null
ln -s /usr/share/metacy /usr/share/metafiks 2> /dev/null
ln -s /usr/share/metacy/loot/workspace /workspace 2> /dev/null
ln -s /usr/share/metacy/loot/workspace /root/workspace 2> /dev/null
ln -s /usr/share/metacy /root/metacy 2> /dev/null
ln -s /root/.metacy.conf /usr/share/metacy/conf/metacy.conf 2> /dev/null
ln -s /root/.metacy_api_keys.conf /usr/share/metacy/conf/metacy_api_keys.conf 2> /dev/null
mv /root/.metacy.conf /root/.metacy.conf.bak 2> /dev/null
cp -vf /usr/share/metacy/metacy.conf /root/.metacy.conf 2> /dev/null
msfdb init 2> /dev/null

echo -e "$OKBLUE[*]$RESET Adding start menu and desktop shortcuts... $RESET"
cp -f $INSTALL_DIR/metafiks.desktop /usr/share/applications/ 2> /dev/null
cp -f $INSTALL_DIR/metafiks.desktop /usr/share/applications/metafiks.desktop 2> /dev/null
cp -f $INSTALL_DIR/metafiks.desktop /usr/share/kali-menu/applications/metafiks.desktop 2> /dev/null
cp -f $INSTALL_DIR/metafiks.png /usr/share/pixmaps/ 2> /dev/null
...
mkdir -p /usr/share/metacy/loot/workspaces/ 2> /dev/null
ln -fs /usr/share/metacy/loot/workspaces/ /home/kali/Desktop/workspaces 2> /dev/null
ln -fs /usr/share/metacy/loot/workspaces/ /root/Desktop/workspaces 2> /dev/null

echo -e "$OKRED[>]$RESET Done! $RESET"
echo -e "$OKRED[>]$RESET To run, type 'metacy'! $RESET"
