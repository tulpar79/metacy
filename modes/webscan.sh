if [[ "$MODE" = "webscan" ]]; then
    ...
    if [[ "$ZAP_SCAN" == "1" ]]; then
    	...
    	sudo python3 /usr/share/metacy/bin/zap-scan.py "http://$TARGET/" 
    	DATE=$(date +"%Y%m%d%H%M")
    	...
    	sudo python3 /usr/share/metacy/bin/zap-scan.py "https://$TARGET/"
	...
	if [[ "$NUCLEI" = "1" ]]; then
		...
		echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
		nuclei -silent -t /usr/share/metacy/plugins/nuclei-templates/ -c $THREADS -target http://$TARGET -o $LOOT_DIR/web/nuclei-http-${TARGET}-port80.txt
		nuclei -silent -t /usr/share/metacy/plugins/nuclei-templates/ -c $THREADS -target https://$TARGET -o $LOOT_DIR/web/nuclei-https-${TARGET}-port443.txt 
	fi
