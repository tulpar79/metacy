...
if [[ "$MODE" = "fullportonly" ]]; then
  if [[ "$REPORT" = "1" ]]; then
    ...
    echo "metacy -t $TARGET -m $MODE --noreport " >> $LOOT_DIR/scans/running_${TARGET}_${MODE}.txt
  ...
  if [[ -f "/usr/share/metacy/pro/.portscanner.conf" ]]; then
    source /usr/share/metacy/pro/.portscanner.conf
  fi
  if [[ -z "$PORT" ]]; then
